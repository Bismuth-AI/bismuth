...
**폴더 하나 = 축 하나.** 하위 폴더들은 **한 질문에 대한 답들**이어야 한다. 첫 부류를 꺼낼 때 축(`Emerging.axis`)을 같이 정해 `split_basis`에 적고, 이후엔 그 축을 프롬프트에 넣어 **같은 질문의 다른 답**만 꺼내게 한다. 안 그러면 형제가 서로 다른 자로 재어져 어떤 이름도 나머지를 배제하지 못한다 — 법령 300건 루트에 `과학기술`(주제)·`시행규칙`(문서 종류)·`제조물 책임법`(법령 이름)이 나란히 섰다.

**조상이 쓴 축은 못 쓴다.** 폴더 안에서 조상의 축은 한 값으로 고정돼 있어 아무것도 구분 못 한다. 안 막았더니 `시행규칙/과학기술정보통신부 소관/시행규칙`과, 그 옆에 `시행령`이 생겼다. 첫 축을 물을 때 조상들의 축을 같이 보여주고 겹치면 거부한다. 자식 이름도 **모든 조상**과 대조한다 — 부모만 보면 위 사례를 못 잡는다. **축은 몇 단어**로 길이를 제한한다(Review가 `basis`에 영문 문단을 써서 축을 잡아먹었다).
...
- **판단 스키마는 근거 필드를 판정보다 앞에 둔다.** 스키마 강제는 **필드를 스키마 순서대로 생성**하므로, 첫 필드는 **아무 생각도 하기 전에** 답해야 하는 자리다. 법령 300건에서 `Emerging.emerged`가 첫 필드였고 **300번 물어 300번 false**가 나왔다 — 같은 응답의 뒤쪽에서 축(`법령의 종류`)과 부류(`법률`)를 정확히 찾아놓고서. thinking을 켜면 JSON 밖에서 미리 생각해 가려지므로, **로컬 모델로 갈 때 드러난다.** `tests/test_decide_after_reasoning.py`가 순서를 못박는다.
