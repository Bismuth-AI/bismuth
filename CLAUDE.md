...
**다시 보는 시점은 증거가 두 배 됐을 때**(`Charter.due_for_review`). 폴더가 자기 내력(`split_basis`, `split_at_documents`)을 노트 frontmatter에 기억한다. 절대 개수가 아니라 자기 대비 비율이라 도메인을 안 타고, *언제 물을지*를 정할 뿐 *어떻게 나눌지*는 모델이 정한다. 재조정이 문서 수에 로그로 늘어나는 이유이기도 하다.
