...
**언제 묻나.** 부류 꺼내기는 직속 문서 수가 2의 거듭제곱일 때(`Charter.due_for_first_look`) — 매번 물으면 같은 13건에 "예" 기회가 13번 생긴다. 경계 재조정은 증거가 두 배 됐을 때(`due_for_review`), 폴더가 자기 내력(`split_basis`, `split_at_documents`)을 노트 frontmatter에 기억한다. **재조정 쪽은 하위 전체로 센다** — 직속으로 세면 나누는 순간 0으로 떨어져 영영 다시 안 본다. 둘 다 *언제 물을지*만 정하고 *어떻게*는 모델이 정한다([SPEC.md §6.1](SPEC.md)). **2배 규칙을 부류 꺼내기에까지 걸면 안 된다** — 걸어봤더니 첫 분할이 그대로 최종 트리가 됐다.
