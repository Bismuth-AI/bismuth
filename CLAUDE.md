...
> **랄프 루프를 돌린다면 매 회 [`docs/ralph/CHECKLIST.md`](docs/ralph/CHECKLIST.md)를 읽는다.**
>
> **원인 분석은 실제 로그를 열어서 한다** ([SPEC.md §6.3](SPEC.md)). 결과 트리·개수·이벤트
> 집계·코드만 보고 원인을 단정하지 않는다. 그것들은 **어디가** 잘못됐는지를 알려줄 뿐이고,
> **왜**는 그 호출의 `system`/`user` 전문과 raw 출력을 봐야 나온다:
> `python scripts/inspect_run.py <로그경로> --folder <폴더>` → 나오는 `# --call <id>` →
> `--call <id>`. 절차는 [`docs/ralph/README.md`](docs/ralph/README.md).
