...
**질문이 둘이고 이게 되풀이를 막는 핵심이다.** 안 나뉜 폴더엔 "나눌 구분이 있나", **이미 나뉜 폴더엔 "그 구분이 아직 맞나"**(기본 답이 "맞다"). 후자에 전자를 던지면 멀쩡한 구조에도 매번 조금씩 다른 답이 나와 문서가 영원히 움직인다.

**다시 보는 시점은 증거가 두 배 됐을 때**(`Charter.due_for_review`). 폴더가 자기 내력(`split_basis`, `split_at_documents`)을 노트 frontmatter에 기억한다. **세는 건 하위 전체지 직속이 아니다** — 나누면 직속이 0으로 떨어져서, 직속으로 세면 나뉜 폴더를 영영 다시 안 본다. 절대 개수가 아니라 자기 대비 비율이라 도메인을 안 타고, *언제 물을지*를 정할 뿐 *어떻게 나눌지*는 모델이 정한다. 재조정이 문서 수에 로그로 늘어나는 이유이기도 하다.

**한 그룹이 전부를 가져가는 건 나눔이 아니다.** 폴더를 한 단계 깊게 옮길 뿐이고 그 아래가 같은 크기의 같은 문제라 무한히 내려간다. 거부하고 `subdivide.rejected`로 로그. `MAX_DEPTH`는 이 논증이 틀렸을 때를 위한 안전망이지 판단 기준이 아니다.
